...
Here we've plotted the sample distribution of $s_b2^$ and $s^2$. The black line indicates the true value of the variance. The blue dashed line is the mean of $s^2$ while the red dashed line is the mean of $s_b^2$. We can see that $s_b^2$ is a biased estimator of the variance, indeed it underestimates the true variance $\sigma^2$.
...
By simulating 1000 values we get above proportions. Observed proportions follow the same order as well as theoretical ones. However there are differences. Because of this difference we tried to calculate theoretical proportions belove and we get similar results as simulation proportions. There might be mistake given proportions.

Assume that the stationary distribution $\pi$ is the vector such that $\pi = \pi Pb$.We can find our stationary distribution by solving the following linear system.
...
subject to $\pi_1+\pi_2+\pi_3=1$. Putting these all equations together and moving all variables to left hand side, we get following linear system.
$$
...
\pi_1 + \pi_2 + \pi_3 = 1
$$
We get the theoretical stationary distribution by solving this linear system.

 Sun  Cloud Rain
0.429 0.285 0.286 

We can get better approximations by increasing the simulation size.
